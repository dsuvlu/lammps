...
FixElectrodeBoundaries::FixElectrodeBoundaries(LAMMPS *lmp, int narg, char **arg) :
  ...
  Vxlo = utils::numeric(FLERR,arg[3],false,lmp);
  dist = utils::numeric(FLERR,arg[4],false,lmp);
  Vxhi = Vxlo + dist;
  // Then voltage@defined plane and voltage difference between electrodes
  v0 = utils::numeric(FLERR,arg[5],false,lmp);
  dv = utils::numeric(FLERR,arg[6],false,lmp);
  etype = utils::inumeric(FLERR,arg[7],false,lmp); //type of atom that is electrochemically active
  seed = utils::inumeric(FLERR,arg[8],false,lmp);
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"region") == 0) { //keyword = region
      ...
      ncycles = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      iarg += 2;
      ...
      neutralIndex = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      iarg += 2;
